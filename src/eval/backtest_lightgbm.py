...
from src.features.config import (
    ...
    DEFAULT_CUTOFFS, DEFAULT_PARAMETERS, HORIZON
)
...
def fit_model_for_windows(X_train: pd.DataFrame, y_train: pd.DataFrame, model_params: dict = DEFAULT_PARAMETERS) -> lgb.LGBMClassifier:

    model = lgb.LGBMRegressor(
        force_col_wise=model_params["force_col_wise"],
...
def run_rolling_backtest(cutoffs: list[str] = DEFAULT_CUTOFFS, horizon: int = HORIZON, model_params: dict = DEFAULT_PARAMETERS) -> tuple[pd.DataFrame, dict]:
...
def main():
    RESULTS_PATH = 'reports/backtest_rolling_windows_lgb.csv'
    SUMMARY_PATH = 'reports/backtest_summary_lgb.json'
